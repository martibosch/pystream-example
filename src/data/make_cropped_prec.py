...
def main():
    ...
    # since" format
    prec_ds = xr.open_mfdataset(
        path.join(paths.external_data_path, 'meteoswiss', 'monthly', 'prec',
    ...
    prec_ds = prec_ds.drop(drop_dims).rename(rename_dims_map)

    # set projection attribute to all data variables
    ds_src = 'epsg:21781'  # ugly hardcoded meteoswiss crs
    prec_ds.attrs['pyproj_srs'] = ds_src
    for data_var in list(prec_ds.data_vars):
        prec_ds[data_var].attrs['pyproj_srs'] = ds_src
    ...
    # file
    roi = prec_ds.salem.roi(dem_ds)
    roi.to_netcdf(path.join(paths.processed_data_path, 'cropped_prec.nc'))
